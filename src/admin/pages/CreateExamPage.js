...
class CreateExamPage extends Component {
  ...
  onCreateRandomExam(e) {
    e.preventDefault();
    if (this.state.nameExam === "" || this.state.time === "" || this.state.bankId === "") {
  ...
  onCreateFixedExam() {
    const listQuestions = this.state.bankData.filter(question => question.selected);
    ...
    if (this.state.nameExam === "" || this.state.time === "" || this.state.bankId === "") {
      ...
      axios
      .post(config.SERVER_URL + '/api/exam/create', {
          bankID: this.state.bankId,
          name: this.state.nameExam,
          time: this.state.time,
      ...
      .then(res => 
      {
        console.log("Tao bank ok" + res.data);
      }
                ...
                <div class="modal-dialog" role="document">
                  <div class="modal-content">
                    ...
                    <div class="modal-footer">
                      ...
                      <button type="submit" class="btn btn-primary" data-dismiss="modal" onClick={(e) => this.onCreateRandomExam(e)}>
                          Tạo ngẫu nhiên
                      </button>
                    </div>
                  </div>
            ...
            <tbody>
              {
                this.state.bankData.map(
                  (question) =>  
                  <tr>
                  <td><input type="checkbox" checked={!!question.selected} onClick={() => this.toggleSelect(question.id)} readOnly></input></td>
                  <td>{question.content}</td>
                  <td>{question.type === 1 ? "Single Choice" : (question.type === 2 ? "Multiple Choice" : "Text Input")}</td>
                  <td>
